...
class WeTypecho_Plugin implements Typecho_Plugin_Interface
{
    public static function activate()
    {
        ...
        //创建公告表
        $hiainotice  = file_get_contents('usr/plugins/WeTypecho/sql/hiai_notice.sql');
        $hiainotice = str_replace('typecho_', $prefix, $hiainotice);
        $hiainotice = explode(';', $hiainotice);
        try {
            if (!$db->fetchRow($db->query("SHOW TABLES LIKE '{$prefix}hitypecho';", Typecho_Db::READ))) {
                foreach ($hiainotice as $script) {
                    $script = trim($script);
                    if ($script) {
                        $db->query($script, Typecho_Db::WRITE);
                    }
