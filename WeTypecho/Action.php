...
class WeTypecho_Action extends Typecho_Widget implements Widget_Interface_Do {
    ...
    private function noticeList() {
        $select   = $this->db->select('gid', 'title', 'content','addtime')->from('notice');
        ...
        if($mid == 99999999) {
            ...
            foreach($posts as $post) {
                ...
                if(sizeof($temp)>0) {
                    ...
                    $temp[0]['thumb'] = $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid']))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])):array(array("str_value"=> $thumb));
            ...
            if(sizeof($cids)>0){
                foreach($cids as $cid) {
                    ...
                    if(sizeof($post)>0 && $post[0]!=null) {
                        ...
                        $thumb = "https://www.hiai.top/usr/themes/HIAI/img/thumb/" . mt_rand(1, 8) . ".jpg";
                        $post[0]['thumb'] = $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $cid))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $cid)):array(array("str_value"=>$thumb));
